...
\item quantify how experimental observation rules affect comparability and synthesize the two reference-domain fidelity profiles;
...
2 & \textbf{Mesoscale predictive fidelity:} kinetics, distributions, topology, morphology, and grain-level response relative to the appropriate reference & established for key simulation outputs; established for experiment in Aim 1; audited for comparability and synthesized in Aim 2 \\
...
\item \textbf{Source-aware fidelity and metric comparability (Aim 2):} measure how experimental observation rules alter nominally common metrics, consolidate simulation fidelity, synthesize the Aim 1 experimental profile, and test effective-time reparameterization without presenting it as physical-time calibration.
...
The remaining research is organized by scientific dependency rather than by manuscript. Aim 1 establishes when experimental learning is trustworthy within the available volume. Aim 2 measures metric comparability and places the simulation and experimental reference-domain results in a common source-aware framework. Aim 3 will interpret only relationships that survive the applicable evidence gates.
...
\textbf{6. Establish the experimental Level 1--2 reliability matrix.} Aim 1 owns the experimental evidence gates at both levels of Table~\ref{tab:evidence-ladder}. The canonical metric families are defined once in Table~\ref{tab:aim1-reliability-matrix}; Aim 2 will import rather than reconstruct this matrix.
...
The primary success criterion is reproducibility across prespecified experimental partitions, not performance in one favorable window. The metric families are those in Table~\ref{tab:aim1-reliability-matrix}; validation will apply the following cross-cutting rules rather than introduce a second metric catalogue:

\begin{itemize}

\item \textbf{Level 1 integrity:} registration residuals will be resolved by axis, using the current no-more-than-0.05-voxel result as a preliminary benchmark rather than an automatically universal threshold, and rollout drift will be judged relative to the uncurated and current curated baselines;
...
\item \textbf{Reproducibility:} uncertainty and pass/partial-pass/fail outcomes will be reported across model replicates and grouped windows or blocks rather than pooled overlapping voxels;
...
\(E\) versus \(\hat{E}\) & Experimental surrogate manuscript and Aim 1 & experimental Level 1--2 metric families and frozen null/readout definitions established in Aim 1 (Table~\ref{tab:aim1-reliability-matrix}) \\
\bottomrule
\end{tabularx}
\end{table}
...
The completed 3D-PRIMME work already provides substantial simulation-domain evidence for kinetics, topology, scaling, voxel accuracy, and prescribed anisotropy. The ongoing experimental work provides direct training, held-out evaluation, and both favorable and failed metrics. Aim 2 will integrate the completed simulation results into a common reporting framework rather than present them as wholly new analysis; its new components are the perturbation-based cross-source metric audit, source-aware synthesis of the Aim 1 experimental profile, effective-time support and sensitivity analysis, and the resulting cross-source claim gate~\cite{tian2026neighborhood,tian2026threeDprimme,tian2026experiment}.

\subsection{Proposed methodology}

\textbf{1. Measure comparability under experimental observation rules.} For each metric in Table~\ref{tab:aim2-metric-policy}, the analysis will first record the object being measured, units or normalization, grain-detection threshold, segmentation and connectivity rule, identity requirement, state/time alignment, and uncertainty source. The audit will then be converted from a definition check into a perturbation measurement. Experimental observation operators \(\mathcal{O}_j\) will be applied to MF trajectories and their surrogate rollouts: resolved-grain cutoffs matched by a declared length-scale mapping or resolved-grain fraction, the experimental connectivity and segmentation/readout rules, and repeated identity-censoring masks calibrated to the observed linkage yields (90.4\% of grains and 97.4\% of voxels for the current training pair). For trajectory \(X\in\{S,\hat S\}\), the metric shift and change in paired discrepancy will be

\begin{equation}
\Delta_m^{(j)}(X)=m\!\left(\mathcal{O}_j X\right)-m(X),
\qquad
\Delta\varepsilon_m^{(j)}
=
d_m\!\left(\mathcal{O}_j S,\mathcal{O}_j\hat S\right)
-d_m\!\left(S,\hat S\right).
\label{eq:comparability-perturbation}
\end{equation}

Absolute shifts, normalized effect sizes, and uncertainty across trajectories or censoring replicates will be reported. A metric will be directly commensurate only if its definition is aligned and its perturbation effects are below a prospectively frozen tolerance; larger but characterizable effects will make it conditionally commensurate, and unstable or non-identifiable effects will make it source specific. Thus the audit can produce an unexpected quantitative result rather than a predetermined classification.
...
\textbf{3. Synthesize the experimental-domain fidelity profile.} Curated trajectories \(E\) and experiment-trained rollouts \(\hat E\) will be selected from the partitions that have already received Level 1--2 pass/partial-pass/fail outcomes in Aim 1. Aim 2 will import the metric, null, linkage, drift, readout, variability, and failure-mode definitions from Table~\ref{tab:aim1-reliability-matrix}; it will not redefine or re-adjudicate the experimental evidence gates. The new task is to express the eligible \(d(E,\hat E)\) results in the same source-aware reporting structure as \(d(S,\hat S)\) while preserving their experimental uncertainties and claim limits.
...
\textbf{5. Produce the source-aware claim matrix.} Each metric will receive its perturbation-based comparability class, within-pair fidelity result, uncertainty, inherited Aim 1 null comparison for \(E\leftrightarrow\hat E\), and pass/partial-pass/fail interpretation. Aim 2 will not serve as a second judge of the experimental evidence it imports: Aim 1 outcomes determine experimental eligibility, whereas Aim 2 determines whether and how an eligible metric may support a cross-source statement. Aim 3 may interpret only Aim 1 eligible experimental outputs; any interpretation that compares or transfers claims across sources must also pass the Aim 2 commensurability gate.
...
The metric definitions, perturbation operators, normalized-effect-size convention, and commensurability tolerances will be frozen before side-by-side synthesis. Simulation and experimental fidelity will be assessed independently through the discrepancies in Equation~\eqref{eq:paired-discrepancies}, with uncertainty across the appropriate trajectories, models, spatial blocks, and censoring replicates. Experimental metrics will retain the null comparisons established in Aim 1. Effective-time results must satisfy the support, marker-consistency, and interpolation-sensitivity rules stated above. Each outcome will receive a pass, partial-pass, or fail interpretation; exact numerical tolerances remain \texttt{[NEEDS QUANTITATIVE CRITERION]}.

\subsection{Expected outcome}

Aim 2 will produce a perturbation-quantified metric-commensurability map, a consolidated simulation fidelity profile, a source-aware synthesis of the Aim 1 experimental profile, an explicit assessment of physical-time versus matched-state conclusions, and a claim matrix. A substantive outcome may be that a familiar metric is reliable within both domains but changes enough under experimental observation rules that it cannot be compared directly across them.

\subsection{Risks}

The experimental pair may support fewer reliable trajectories and less precise uncertainty than the simulation pair. Nominally common metrics may prove non-equivalent after detection, connectivity, and linkage perturbations are measured. The MF and experimental data may lack a unique physical length-scale mapping for the detection-limit perturbation; in that case, resolved-grain-fraction sensitivity will be reported instead. The five experimental states may be insufficient for a stable effective-time mapping, and aggregate metrics may hide local failure within either pair.

\subsection{Alternative strategies}

If a metric cannot be harmonized or its perturbation effect is unstable, it will remain source specific rather than be forced into a common ranking. If too few distinct rollout steps fall within the experimental marker range, mappings disagree across state markers, or results are sensitive to interpolation, no effective-time conclusion will be reported and only matched-state and raw-time results will remain. If grain linkage is inadequate, identity-agnostic metrics will replace individual-grain metrics. Failed within-domain criteria will remain scientific results and will prevent the associated claim from advancing to Aim 3.

\subsection{Deliverables}

\begin{itemize}
\item a perturbation-quantified cross-source metric-comparability map;
\item a consolidated simulation-reference versus simulation-trained fidelity profile;
\item a source-aware synthesis of the experimental-reference versus experiment-trained fidelity profile established across Aim 1 eligible partitions;
...
Aim 1 establishes experimental Levels 1--2 integrity, predictive fidelity, null comparison, and spatial reproducibility within the available volume. Aim 2 quantitatively audits cross-source metric comparability, consolidates the simulation Level 2 profile, and synthesizes the eligible experimental profile under a common claim policy without re-adjudicating Aim 1. Aim 3 advances only Aim 1 eligible outputs to Level 3 model interpretation; statements that compare or transfer evidence across sources must also pass the Aim 2 commensurability gate before supporting Level 4 physical attribution or Level 5 candidate-mechanism testing.
...
The program is efficient because the same local architecture, data representation, rollout code, replicate policy, and evidence ladder are reused across stages. Aim 3 implementation and diagnostic development may overlap with Aim 2, but final experimental interpretation will be restricted to Aim 1 eligible outputs, and cross-source interpretation will additionally require Aim 2 commensurability. What changes across stages is the supervision source and the strength of the claim allowed by the validation.
...
March 2027 & Complete perturbation effect sizes, paired-profile synthesis, uncertainty, and failure-mode analyses & Finalize Aim 2 figures, claim matrix, and chapter & Which eligible metrics support a cross-source statement? \\

February-April 2027, overlapping & Begin provisional Aim 3 implementation, then restrict final analyses to Aim 1 eligible outputs and Aim 2 commensurate cross-source claims & Complete Aim 3 figures, evidence matrix, and synthesis & Which signatures support Level 3 interpretation, Level 4 attribution, or Level 5 testing? \\
...
\item Final Aim 2 reference trajectories, observation-rule perturbations, effect-size normalization and commensurability tolerances, effective-time support/interpolation policy, replicate policy, and quantitative criteria. \texttt{[AUTHOR INPUT REQUIRED]} \texttt{[NEEDS QUANTITATIVE CRITERION]}
