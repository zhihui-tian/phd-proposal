...
The proposal does not address cross-specimen, cross-processing-condition, or cross-material generalization. It also does not guarantee a continuously calibrated physical-time evolution law, correction of late-time kinetics, or formal probabilistic uncertainty calibration when the number of sufficiently independent windows is inadequate. Finally, direct experimental supervision alone cannot establish that the learned operator contains specific material physics absent from MF, which would require a separately calibrated simulation--experiment comparison, nor can it guarantee unique physical-mechanism discovery. Aim 3 may instead establish stable model interpretation or bounded physical attribution.
...
The primary success criterion is reproducibility across prespecified experimental partitions, not performance in one favorable window. Registration residuals will be resolved by axis and rollout drift judged against the uncurated and current curated baselines. Each applicable metric family must improve prospectively over its frozen null, remain stable under the prespecified sensitivity analyses, and be reported across model replicates and grouped windows or blocks rather than pooled overlapping voxels. Any abstention rule must improve retained performance while reporting its coverage. Exact pass/fail thresholds will be fixed after the number and quality of additional windows are known. \texttt{[NEEDS QUANTITATIVE CRITERION]}

The expected outcome is an empirical domain of applicability that states where inside the available volume the operator can and cannot be trusted. A narrow or empty predictive domain remains informative if the warning framework identifies the failure instead of masking it.

The principal contingency is that the available volume, especially its unavoidable \(y\)-direction overlap, may yield too few sufficiently independent, high-quality windows for cross-window validation. In that case, Aim 1 will quantify within-volume reproducibility and data-quality limits rather than claim independence. If grain identities cannot be retained, identity-agnostic population and boundary metrics will replace individual-grain tests; if systematic effects or sample scarcity preclude probabilistic calibration, uncertainty will be reported empirically. In all cases, Aim 1 will produce a curated dataset with its quality record and a reliability framework that defines the supported claim domain.
...
Third, for each family the reported quantity will be the improvement over its frozen null baseline together with the associated uncertainty, not the raw metric alone. A family whose value does not separate from its null will be recorded as uninformative rather than as a pass. Each metric family will then receive a pass, partial-pass, or fail interpretation together with its null comparison, uncertainty, and the stated reason for the outcome. A failed family is excluded from Aim 3 interpretation but is retained and reported, never removed from the record.
...
The protocol, tolerances, and evaluated partition will be frozen before the held-out data are examined; any post hoc revision will reduce the affected result to exploratory status. Each metric family must improve prospectively over its frozen null, with uncertainty estimated over model replicates and spatial blocks rather than individual voxels. Effective-time results must also satisfy the support, marker-consistency, and interpolation-sensitivity rules stated above. Each family will receive a pass, partial-pass, or fail interpretation; exact tolerances remain \texttt{[NEEDS QUANTITATIVE CRITERION]} until the Aim 1 eligible-partition set is known.

The most likely result is a partial pass in which field morphology, early kinetics, and normalized size statistics separate from their nulls while topology or individual-grain response does not. The resulting validation matrix will fix the boundary of what the operator may be used to claim and which outputs Aim 3 may interpret.

The principal contingency is that the available volume may not contain a training-blind partition large enough for stable statistics. If so, the result will be reported as a blocked within-volume test, not as independent validation. If linkage is inadequate, identity-agnostic metrics will replace individual-grain metrics for that family; insufficient temporal support will preclude an effective-time conclusion. Any failed or unsupported family will be retained in the record and excluded from Aim 3 rather than replaced by a more favorable metric.
...
Interpretation summaries will be reported across model replicates with bootstrap or replicate-based uncertainty. Input-side conclusions require agreement between scale-mixer rankings and at least one functional ablation, while output-side conclusions must be stable to sample size, signed-versus-absolute aggregation, cubic coordinate transformations, and the eligible experimental-window partition. Physical attribution additionally requires a consistent effect across reliable windows and replication in a held-out window or controlled simulation. These criteria correspond to Levels 3--4 of Table~\ref{tab:evidence-ladder}; Level 5 requires a falsifiable prediction or intervention. Exact tolerances will be frozen after the Aim 1 eligible-window set is known and before the final Aim 3 comparison.
...
\midrule
Aim 1 reliability & Model behavior depends strongly on one window or overlap block & Narrow the valid domain and withhold broader spatial-reproducibility claims \\
Aim 2 validation & A metric family fails the training-blind test or does not separate from its frozen null & Record the family as failed or uninformative and exclude that output from physical interpretation \\
...
Conversely, if every metric family passes, the result establishes validated fidelity within the evaluated experimental domain but does not by itself prove that the operator has recovered material physics absent from the simulator.
