...
\item \textbf{Training-blind system validation (Aim 2):} apply the definitions and null baselines frozen in Aim 1, without modification, to an experimental partition that contributed to neither training nor model selection, and adjudicate each metric family as pass, partial pass, or fail; effective-time reparameterization will be reported as a support-limited diagnostic rather than physical-time calibration.
...
Experiment-trained operators that satisfy the Aim 1 reliability gates will reproduce field morphology, early coarsening kinetics, and normalized grain-size statistics on a training-blind partition at a level that exceeds their prespecified null baselines, whereas topology and individual-grain response will remain the metric families most likely to fail. The adjudication is therefore expected to be partial rather than uniform, and the resulting pattern of passes and failures will define the domain within which the operator may subsequently be interpreted.

\subsection{Relationship to Aim 1: exploratory versus confirmatory evidence}

Aim 1 and Aim 2 evaluate the same metric families but occupy different evidentiary roles. The separation between them is what allows an Aim 2 outcome to be reported as system validation rather than as a further diagnostic.
...
The ongoing experimental study provides direct T0-to-T1 training, three initialization replicates, held-out T2--T4 evaluation, grain-size and topology comparisons, individual-grain tests where linkage permits, and directional rollout-drift analysis~\cite{tian2026experiment}. These results establish that the evaluation is executable and that both favorable and unfavorable outcomes can be measured. They do not constitute system validation, for the two reasons given above.
...
\subsection{Optional exploration beyond the core aim: cross-source metric comparability}

The analysis in this subsection is retained as optional exploration. It is not part of the core dissertation plan, contributes nothing to the Aim 2 validation claim, and will be undertaken only if the core aims are completed with time remaining. No result in Chapters 4--6 depends on it, and the simulation-trained surrogate therefore remains a completed tool and controlled baseline rather than a component of the experimental validation claim.

Four data products support two within-source fidelity comparisons:
...
The ordered collections \(\boldsymbol{\varepsilon}^{(S)}\) and \(\boldsymbol{\varepsilon}^{(E)}\) form separate fidelity profiles; the exploration would not combine them into one scalar score. It would ask instead whether the definitions underlying a nominally common metric are equivalent enough for two profile entries to be interpreted together, classifying each nominally common metric as directly commensurate, conditionally commensurate, or source specific. That classification has an unknown answer: a metric may fail comparability even when each surrogate reproduces it within its own source.

The classification would be measured rather than asserted. Experimental observation operators \(\mathcal{O}_j\) would be applied to MF trajectories and their surrogate rollouts, using resolved-grain cutoffs matched by a declared length-scale mapping or resolved-grain fraction, the experimental connectivity and segmentation rules, and repeated identity-censoring masks calibrated to the observed linkage yields. For trajectory \(X\in\{S,\hat S\}\),
...
The core plan does not use \(d(S,E)\) as evidence for physics missing from simulation. A direct MF rollout initialized from an experimental state would require its own parameter calibration, boundary-condition treatment, state alignment, and uncertainty model, and it remains outside this dissertation. Consequently the dissertation tests experimental system validation and, optionally, metric commensurability; it does not test whether experimental supervision has isolated particular material mechanisms absent from MF.
...
Aim 1 establishes experimental Levels 1--2 integrity, predictive fidelity, null comparison, and spatial reproducibility within the available volume. Aim 2 applies those frozen definitions once to a training-blind experimental partition and adjudicates each metric family as pass, partial pass, or fail, without re-adjudicating Aim 1. Aim 3 advances only Aim 1 eligible and Aim 2 validated outputs to Level 3 model interpretation before supporting Level 4 physical attribution or Level 5 candidate-mechanism testing; statements that compare or transfer evidence across sources lie outside the core plan.
...
\item If a family passes but does not separate from its frozen null, it will be recorded as uninformative rather than as evidence of predictive fidelity.
...
The program is efficient because the same local architecture, data representation, rollout code, replicate policy, and evidence ladder are reused across stages. Aim 3 implementation and diagnostic development may overlap with Aim 2, but final experimental interpretation will be restricted to outputs that are both Aim 1 eligible and Aim 2 validated. What changes across stages is the supervision source and the strength of the claim allowed by the validation.
...
\item a frozen, training-blind validation protocol for experiment-trained rollout that keeps fidelity to the simulation teacher distinct from fidelity to experiment and records failed metrics rather than averaging them away;
...
To protect the core scope, the schedule prioritizes Aim 1 window-feasibility and spatial-reproducibility analysis, preserves the Aim 2 training-blind validation, and bases final Aim 3 claims on validated outputs. Cross-source metric comparability, external datasets, new materials, a direct MF-from-experimental-initial-state comparison, broad architecture sweeps, and full probabilistic calibration remain outside the core plan.
...
\item Final Aim 2 evaluated partition, per-family tolerances, null-comparison and uncertainty estimators, effective-time support/interpolation policy, replicate policy, and the protocol-freeze date. \texttt{[AUTHOR INPUT REQUIRED]} \texttt{[NEEDS QUANTITATIVE CRITERION]}
